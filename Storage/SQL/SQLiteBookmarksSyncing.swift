...
extension SQLiteBookmarks {
    ...
    fileprivate func insertBookmarkInTransaction(_ deferred: Success, url: URL, title: String, favicon: Favicon?, intoFolder parent: GUID, withTitle parentTitle: String, conn: SQLiteDBConnection, err: inout NSError?) -> Bool {
        ...
        log.debug("Inserting bookmark in transaction on thread \(Thread.currentThread())")
        ...
        log.debug("Returning true to commit transaction on thread \(Thread.currentThread())")
    ...
    func insertBookmark(_ url: NSURL, title: String, favicon: Favicon?, intoFolder parent: GUID, withTitle parentTitle: String) -> Success {
        log.debug("Inserting bookmark task on thread \(Thread.currentThread())")
        ...
        log.debug("Returning deferred on thread \(Thread.currentThread())")
        return deferred
    }
}
...
open class SQLiteBookmarkBufferStorage: BookmarkBufferStorage {
    ...
    func change(_ conn: SQLiteDBConnection, sql: String, args: Args?, desc: String) -> Bool {
    ...
    open func reorderBookmarks(_ folderGUID:String, bookmarksOrder:[String], completion:@escaping ()->())  {
        var err: NSError?
    ...
    open func editBookmark(_ bookmark:BookmarkNode, newTitle:String?, newParentID:String?, completion:@escaping ()->())  {
    ...
    open func createFolder(_ folderName:String, completion:@escaping ()->()) {
        ...
        structureArgs.append(parentGUID as AnyObject?)
        ...
        let local =
            "INSERT INTO \(TableBookmarksLocal) " +
                ...
                Array(repeating: "(?, ?, ?, ?, '', ?, ?)", count: 1).joined(separator: ", ")
...
extension MergedSQLiteBookmarks: BookmarkBufferStorage {
    ...
    public func editBookmark(_ bookmark:BookmarkNode, newTitle:String?, newParentID: String? = nil, completion:@escaping ()->())  {
        self.buffer.editBookmark(bookmark, newTitle:newTitle, newParentID:newParentID, completion:completion)
    }
    
    public func reorderBookmarks(_ folderGUID:String, bookmarksOrder:[String], completion:@escaping ()->())  {
        self.buffer.reorderBookmarks(folderGUID, bookmarksOrder:bookmarksOrder, completion:completion)
    }

    public func createFolder(_ folderName:String, completion:@escaping ()->())  {
