...
open class MockLogins: BrowserLogins, SyncableLogins {
    ...
    open func getLoginsForProtectionSpace(_ protectionSpace: NSURLProtectionSpace) -> Deferred<Maybe<Cursor<LoginData>>> {
    ...
    open func getLoginsForProtectionSpace(_ protectionSpace: NSURLProtectionSpace, withUsername username: String?) -> Deferred<Maybe<Cursor<LoginData>>> {
        let cursor = ArrayCursor(data: cache.filter({ login in
            return login.protectionSpace.host == protectionSpace.host &&
    ...
    open func markAsSynchronized(_: [GUID], modified: Timestamp) -> Deferred<Maybe<Timestamp>> { return deferMaybe(0) }
    open func markAsDeleted(_ guids: [GUID]) -> Success { return succeed() }
