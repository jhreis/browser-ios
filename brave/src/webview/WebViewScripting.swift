...
class LegacyUserContentController
{
    ...
    func injectIntoMain() {
        ...
        let result = webView.stringByEvaluatingJavaScript(from: "window.hasOwnProperty('webkit') && webkit.hasOwnProperty('messageHandlers') && window.hasOwnProperty('__firefox__')")
        if result == "true" {
        ...
        for (name, handler) in scriptHandlersMainFrame {
            if !name.lowercased.contains("reader") {
