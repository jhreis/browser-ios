...
class AuralProgressBar {
    fileprivate class UI {
        ...
        @objc func handleAudioSessionInterruptionNotification(_ notification: Notification) {
            if let interruptionTypeValue = (notification as NSNotification).userInfo?[AVAudioSessionInterruptionTypeKey] as? UInt {
                if let interruptionType = AVAudioSessionInterruptionType(rawValue: interruptionTypeValue) {
                    ...
                    case .began:
                        ...
                        progressPlayer.stop()
                    case .ended:
                        if let interruptionOptionValue = (notification as NSNotification).userInfo?[AVAudioSessionInterruptionOptionKey] as? UInt {
